...
class Api::ProductListingsController < ApplicationController
  ...
  def index
    ...
    @product_listings = ProductListing.where(seller_id: @current_user.id)
  ...
  def product_listing_params
    params.require(:product_listing).permit(:id, :seller_id, :product_id, :price, :condition, :size, :listing_type)
  end
end
